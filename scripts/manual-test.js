  ...
  try {
    ...
    Object.entries(byUserId).forEach(([userId, userTransactions]) => {
      ...
      userTransactions.slice(0, 3).forEach(t => {
        console.log(`  - ${t.type}: ${t.amount} (${t.category}) - ${t.description}`);
      });
    });
    ...
    if (adminUser) {
      ...
      const adminTransactions = await db.collection("transactions").find({ 
    ...
    sessions.forEach(session => {
      console.log(`- User: ${session.username} (${session.userId?.toString()}), Expires: ${session.expiresAt}`);
    });
